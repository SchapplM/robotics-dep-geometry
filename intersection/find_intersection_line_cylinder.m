...
function pts = find_intersection_line_cylinder(p, u, p1, p2, r)
 ...
 assert(isa(r,'double') && isreal(r) && all(size(r) == [1 1]), ...
  ...
  % Berechne Lotfu�punkt q der windschiefen geraden g1: x=p+s*u; s aus R und
  % g2: x=p1+t*v; v=p2-p1, t aus R auf der geraden g1. Dazu wird die
  % Hilfsebene E: x=p1+s*v+t*(u x v) konstruiert, die g2 enth�lt und als
  % zweite Richtung die senkrechte zu beiden geraden (u x v) enth�lt. Der
  ...
  % von g1 hier ein, kann man den parameter s f�r den Lotfu�punkt
  ...
  % n�chstgelegene Punkt zum Zylinder bestimmt. Wenn Schnittpunkte vorliegen,
  % wird gepr�ft, ob Sie auf dem zum Zylinder geh�rigen Teil des unendlichen
  % Zylinders liegen. Das ist der Fall, wenn (q-p2)*v<0 und
  % (q-p1)*v>0 ist. Ist die erste Bedingung verletzt, gibt es
  % (mindestens) eine Schnittpunkt mit der Randfl�che um p1, ist die zweite
  % Bedingung verletzt, gibt es (mindestens) eine Schnittpunkt mit der
  % Randfl�che um p2. Dies folgt daraus, dass die Skalarprodukte die
  % relative Richtung der Projektion des Vektors von pi nach q auf den
  % Vektor von p1 nach p2 angibt. F�r die beiden Randfl�chen wird, falls
  ...
  % und anschlie�end gepr�ft, ob |qi-pi|<r gilt.
  % (p+s*u-p1)*v=0 <=> s = (p1-p)*v/(u*v)
  % f�r die Randfl�che um p1 und
  % s = (p2-p)*v/(u*v)
  % f�r die Randfl�che um p2.
  


  cpp1v  = cross(p-p1,v);
  % Wenn keine Schnittpunkte vorliegen, muss der Punkt auf dem Zylinder
  % berechnet werden, der der Geraden g1 am n�chsten liegt. Dazu werden
  % wieder die drei F�lle "q jenseits von p1", "q jenseits von p2" und "q
  % zwischen p1 und p2" unterschieden. Fall 1 und 2: l�sst sich nur durch
  % eine Minimierungsaufgabe l�sen. Dazu nutzen wir das Verfahren nach
  % dem goldenen Schnitt.
  % Fall 3: wir kennen die Richtung von q nach g2: u x v. Den Lotfu�punkt
  % q3 auf g2 k�nnen wir durch eine Projektion von q-p1 auf v berechnen:
  % q3 = p1+(q-p1)*v/v^2*v von da aus gehen wir um die L�nge r in
  ...
  if norm(cuv)/(norm(u)*norm(v))<1e-10 % u parralel zu Zylinderachse
    ...
    if norm(r1)>=r
      ...
      pts = correct_pts(pts);
      return;
    else % Schnittpunkte auf beiden Zylinderflaechen
      pts = [p1+r1 p2+r1];
      return;
    end
  end
  ...
  if radikant < 0 || isnan(radikant)
    % kein Schnittpunkt liegt vor, w�hle n�chsten Punkt
    pc1 = find_next_pkt(q,p1,p2,p,v,u,r);
    pts = [pc1, [norm(cross(pc1-p,u))/norm(u); NaN(2,1)]];
    pts = correct_pts(pts);
    return;
  else % Schnittpunkte gefunden, auf Korrektheit pr�fen und ggf. korrigieren
    ...
    pc1 = p+s1*u;
    pc2 = p+s2*u;
    if abs(u.'*v/(norm(u)*norm(v)))<1e-10 % g parallel zur Zylinderflaeche
    ...
    if (pc1-p1).'*v<0 % Schnittpunkt mit Zylinderflaeche um p1 liegt vor
      [pc1,pts,finished] = correct_intersection_k1(q,p1,p2,p,v,u,r);
      if finished
        pts = correct_pts(pts);
        return;
      end
    elseif (pc1-p2).'*v>0 % Schnittpunkt mit Zylinderflaeche um p2 liegt vor
...
% Hilfsfunktion zur Korrektheitspr�fung und Korrektur der
...
function [p_out,pts,finished] = correct_intersection_k1(q,p1,p2,p,v,u,r)
  ...
  s = (p1-p).'*v/(u.'*v);
  p_out = p+s*u;
  if (p1-p_out).'*(p1-p_out)>r^2 || any(isnan(p_out)) % falls keine Schnittpunkte vorliegen: n�chster pkt
...
function [p_out,pts,finished] = correct_intersection_k2(q,p1,p2,p,v,u,r)
  ...
  s = (p2-p).'*v/(u.'*v);
  p_out = p+s*u;
  if (p2-p_out).'*(p2-p_out)>r^2 || any(isnan(p_out)) % falls keine Schnittpunkte vorliegen: n�chster pkt
    p_out = find_next_pkt(q,p1,p2,p,v,u,r);
    pts = [p_out [norm(cross(p_out-p,u))/norm(u); NaN(2,1)]];
    finished=1;
    return;
...
% Hilfsfunktion zum Finden des n�chstgelegenen Punktes auf der
% Zylinderoberfl�che
function p_out = find_next_pkt(q,p1,p2,p,v,u,r)
  if (q-p1).'*v<0
    if (abs(u.'*v/(norm(u)*norm(v)))<1e-10) % gerade g parallel zu zylinderoberflaeche
      p_c = p+(p1-p).'*u/(u.'*u)*u;
    else
      p_in = p+(p1-p).'*v/(u.'*v)*u;
...
% Kreisscheibe. Die Idee ist, zunaest den Abstand des Punktes zur
% Kreisebene l1 zu berechnen und anschlie�end den Abstand in der Ebene l2
