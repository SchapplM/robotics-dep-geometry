...
% Eingabe:
% Ebene [1x9]
%   Darstellung einer Ebenen (Punkt 1, Richtungsvektor 1, Richtungsvektor 2)
% Punkt [1x3]
...
% 
% Siehe auch: distance_line_line, distance_line_point
% 
% Quelle:
...
function [dnorm, d, pe] = distance_plane_point(Ebene, Punkt)

%% Coder Information
%#codegen
assert(isa(Ebene,'double') && isreal(Ebene) && all(size(Ebene) == [1 9]) && ... 
       isa(Punkt,'double') && isreal(Punkt) && all(size(Punkt) == [1 3]));

%% Algorithmus
y = Punkt';
p = Ebene(1:3)';
a = cross(Ebene(4:6)', Ebene(7:9)');
